pipeline {
    ...
    stages {
        ...
        stage ('AWS Login') {
            steps {
               sh 'aws ecr get-login-password --region ap-south-1 | docker login --username AWS --password-stdin 862839345820.dkr.ecr.ap-south-1.amazonaws.com/docker-deploy-1'
            }
        }
        stage ('AWS Deploy') {
            steps {
